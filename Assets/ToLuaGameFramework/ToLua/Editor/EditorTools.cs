...
namespace LuaInterface.Editor
{
    ...
    public static class EditorTools
    {
        ...
        /// 注意：包括所有子目录的文件
        /// </summary>
        public static void CopyDirectoryFileMap(string sourcePath, string destPath, string searchPattern = "*.*", Func<string, string> fnFileMap = null)
        {
            if (!destPath.EndsWith("/") && !destPath.EndsWith("\\")) destPath += "/";
            sourcePath = EditorTools.GetRegularPath(sourcePath);
            ...
            string[] fileList = Directory.GetFiles(sourcePath, searchPattern, SearchOption.AllDirectories);
            foreach (string file in fileList)
            {
                string temp = EditorTools.GetRegularPath(file);
                string savePath = temp.Replace(sourcePath, destPath);
                if (fnFileMap != null) {
                    savePath = fnFileMap(savePath);
                }
                CopyFile(file, savePath, true);
            }
        }
        ...
        public static string GetTargetPlatform(UnityEditor.BuildTarget target)
        {
